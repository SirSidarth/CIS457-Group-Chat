...
class ChatClientApp:
    def __init__(self, master):
        ...
        self.chat_log.tag_configure("me", justify="right", background="#d0f0c0") # Light green, messages client sent
        self.chat_log.tag_configure("other", background="white") # White, messages from other clients
        self.chat_log.tag_configure("connection", background="#e6f7ff") # Light blue, connection messages
        self.chat_log.tag_configure("error", background="#ffb3b3") # Light red, error messages

        self.message_entry = tk.Text(master, height=5, width=40)

        self.send_button = tk.Button(master, text="Send", command=self.send_message)

        # Disable the message system until the username and conneciton are set
    ...
    def prompt_username(self): # Asks user to put in a name before connecting
        ...
        def set_username():
            ...
            if name and (name != "Me"):
                status = self.connect_to_server(name)
                if status == 0:
                    self.username = name
                    popup.destroy()
                    ...
                    error_label.config(text="Failed to connect to server")
            else:
    ...
    def receive_messages(self):
        while self.running:
            try:
                ...
                self.send_button.config(state='disabled') # Client will no longer be able to send messages, can still view history until program is closed
                break
    ...
    def append_message(self, message):
        ...
        if message[:1] == "0": # numbers indicate the type of message sent
            tag = "other"
            message = message[1:]
